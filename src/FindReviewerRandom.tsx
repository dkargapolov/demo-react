...
export const FindReviewerRandom = ({ login, repo, blacklist }: FindReviewerProps) => {
  ...
  const findReviewer = async () => {
    ...
    if (!login || !repo) {
      setError({
        type: "validation",
        message: "Необходимо указать логин и репозиторий"
      });
      setLoading(false);
      return;
    }
  ...
  const renderError = (errorInfo: ErrorInfo) => {
    ...
    const hints = {
      ...
      validation: "💡 Проверьте переданные параметры login и repo.",
  ...
  return (
    <div className="find-reviewer">
      <div className="find-reviewer__info">
        <p className="find-reviewer__info-text">
          <strong>Поиск ревьюера для:</strong> {repo}
        </p>
        <p className="find-reviewer__info-text">
          <strong>Текущий пользователь:</strong> {login || "не указан"}
        </p>
        {blacklist.length > 0 && (
          <p className="find-reviewer__info-text">
            <strong>Исключены:</strong> {blacklist.join(", ")}
          </p>
        )}
      </div>
